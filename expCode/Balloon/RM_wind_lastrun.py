...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\balloon\\RM_PsychopyBuilder\\RM_wind_lastrun.py',
...
image = visual.ImageStim(
    ...
    image='balloon-blue.png', mask=None,
...
trials_2 = data.TrialHandler(nReps=8, method='random', 
...
for thisTrial_2 in trials_2:
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            baseline = pos_jit + start_pos*15+2*-7.5*start_pos
        ...
        # keep track of which components have finished
        respComponents = [polygon_2, polygon_3, key_resp, mouseTrial]
        ...
        while continueRoutine:
            ...
            if polygon_2.status == STARTED:  # only update if drawing
                polygon_2.setPos((baseline+x_def, 2.5))
            ...
            if polygon_3.status == STARTED:  # only update if drawing
                polygon_3.setPos((baseline+x_def, -1.5))
            ...
            if mouseTrial.status == NOT_STARTED and t >= 0.0-frameTolerance:
                ...
                prevButtonState = mouseTrial.getPressed()  # if button is down already this ISN'T a new click
            if mouseTrial.status == STARTED:  # only update if started and not finished!
                x, y = mouseTrial.getPos()
                mouseTrial.x.append(x)
                mouseTrial.y.append(y)
                buttons = mouseTrial.getPressed()
                mouseTrial.leftButton.append(buttons[0])
                mouseTrial.midButton.append(buttons[1])
                mouseTrial.rightButton.append(buttons[2])
                mouseTrial.time.append(mouseTrial.mouseClock.getTime())
                buttons = mouseTrial.getPressed()
                if buttons != prevButtonState:  # button state changed?
                    prevButtonState = buttons
                    if sum(buttons) > 0:  # state changed to a new click
        ...
        trials.addData('mouseTrial.x', mouseTrial.x)
        trials.addData('mouseTrial.y', mouseTrial.y)
        trials.addData('mouseTrial.leftButton', mouseTrial.leftButton)
        trials.addData('mouseTrial.midButton', mouseTrial.midButton)
        trials.addData('mouseTrial.rightButton', mouseTrial.rightButton)
        trials.addData('mouseTrial.time', mouseTrial.time)
        trials.addData('mouseTrial.started', mouseTrial.tStart)
        trials.addData('mouseTrial.stopped', mouseTrial.tStop)
...
# completed 8 repeats of 'trials_2'
