...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\RM\\TTC_sample3_lastrun.py',
...
text = visual.TextStim(win=win, name='text',
    text='same = F\ndifferent = J\n\n(space)',
    font='Open Sans',
    units='deg', pos=(0, 5), height=1.0, wrapWidth=None, ori=0.0, 
...
fix = visual.TextStim(win=win, name='fix',
    text='+',
    font='Open Sans',
    units='deg', pos=(0, 5), height=1.5, wrapWidth=None, ori=0.0, 
    color=(0.0000, 0.0000, 0.0000), colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-1.0);
...
trials = data.TrialHandler(nReps=100.0, method='random', 
    ...
    trialList=data.importConditions('TTC_sample3.csv'),
...
for thisTrial in trials:
    ...
    start_size = 1
    # keep track of which components have finished
    trialComponents = [fix, polygon, polygon_2]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if fix.status == STARTED:
            ...
            if tThisFlipGlobal > fix.tStartRefresh + 1.75-frameTolerance:
        ...
        if polygon_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > polygon_2.tStartRefresh + 0.75-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if polygon_2.status == STARTED:  # only update if drawing
            polygon_2.setSize((11+ size2, 11 + size2))
