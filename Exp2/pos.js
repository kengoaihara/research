...
function restOrNotLoopBegin(restOrNotLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisRestOrNot of restOrNot) {
...
function pra_instRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of pra_instComponents)
...
function praRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of praComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function noise_3RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of noise_3Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function main_instRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of main_instComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
